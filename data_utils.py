...
class Dataset:
    ...
    def evaluate_recall(self):
        ...
        total_ex = len(self.query_examples) # 查询示例的总数
...
def build_happy_dataset(dataset_name: str, tokenizer: Any, batch_size: int = 10000) -> Dataset:
    ...
    for i in range(20):  # 从0到19
        file_name = "/home/zt/data/open-images/train/processed_nn2/response_results_batch_{}.json".format(i)
        if glob.glob(file_name):  # 检查文件是否存在
            with open(file_name) as f:
                queries.extend(json.load(f))
    index_img_ids = json.load(open(f"/home/zt/data/open-images/train/processed_nn2/index.json"))
    index_image_folder = "/home/zt/data/open-images/train/data"
